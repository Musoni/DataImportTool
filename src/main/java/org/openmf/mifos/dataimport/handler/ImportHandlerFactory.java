...
public class ImportHandlerFactory {
    ...
    public static final DataImportHandler createImportHandler(Workbook workbook, HttpServletRequest request) throws IOException {
        
        if(workbook.getSheetIndex("Clients") == 0) {
            	return new ClientDataImportHandler(workbook, new MifosRestClient(request));
        }else if(workbook.getSheetIndex("Groups") == 0) {
    	    return new GroupDataImportHandler(workbook, new MifosRestClient(request));
        }else if(workbook.getSheetIndex("Centers") == 0) {
    	    return new CenterDataImportHandler(workbook, new MifosRestClient(request));
        }else if(workbook.getSheetIndex("Loans") == 0) {
        	    return new LoanDataImportHandler(workbook, new MifosRestClient(request));
        } else if(workbook.getSheetIndex("LoanRepayment") == 0) {
        	    return new LoanRepaymentDataImportHandler(workbook, new MifosRestClient(request));
        } else if(workbook.getSheetIndex("Savings") == 0) {
    	    return new SavingsDataImportHandler(workbook, new MifosRestClient(request));
        } else if(workbook.getSheetIndex("SavingsTransaction") == 0) {
    	    return new SavingsTransactionDataImportHandler(workbook, new MifosRestClient(request));
        } else if(workbook.getSheetIndex("FixedDeposit") == 0) {
        	return new FixedDepositImportHandler(workbook, new MifosRestClient(request));
        } else if(workbook.getSheetIndex("RecurringDeposit") == 0) {
        	return new RecurringDepositImportHandler(workbook, new MifosRestClient(request));
        } else if(workbook.getSheetIndex("RecurringDepositTransaction") == 0) {
        	return new RecurringDepositAccountTransactionDataImportHandler(workbook, new MifosRestClient(request));
        }else if(workbook.getSheetIndex("ClosingOfSavingsAccounts") == 0) {
            	return new ClosingOfSavingsAccountHandler(workbook, new MifosRestClient(request));
        }else if(workbook.getSheetIndex("AddJournalEntries") == 0) {
        	return new AddJournalEntriesHandler(workbook, new MifosRestClient(request));
        }else if(workbook.getSheetIndex("guarantor") == 0) {
        	return new AddGuarantorDataImportHandler(workbook, new MifosRestClient(request));
        }else if(workbook.getSheetIndex("ClientIdentifiers") == 0) {
        	return new ClientIdentifierDataImportHandler(workbook, new MifosRestClient(request));
        }
